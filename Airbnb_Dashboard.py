...
# change the path to the location where the project folder is stored
path = "G:/Meine Ablage/Python Programming Projekt/" #Pfad Fabio
#path = "/Volumes/GoogleDrive/.shortcut-targets-by-id/1gXnbZ_RfE3F_bjdRwEzzVoMBKQG6Basi/Python Programming Projekt/" #Pfad Damian
#path = "G:/.shortcut-targets-by-id/1gXnbZ_RfE3F_bjdRwEzzVoMBKQG6Basi/Python Programming Projekt/" #Pfad Cedric 

# now open your anaconda prompt.
# run the next line of code (line 20) and copy/paste the console output to the anaconda prompt
# now press enter in the prompt. This should open your standard browser with the streamlit dashboard
print('streamlit run ' + '"' + path + 'Airbnb_Dashboard.py"')

################################################
# Load Libraries, the data and prediction model
################################################
...
#change desing of some default settings from streamlit like a menu bar, footer and header.
hide_header_style = """
   <style>
...
# Define Load functions for the datasets of the dashboard. All in cache -> Does not always have to be reloaded.
# load base data
...
# load the data for the data analyzing section
df = load_data()

# load the data & model for the prediction section
df_predict = copy.deepcopy(load_prediction_data())
model = load_model()


################################################
# Additional preperation for the program
################################################

# Loads all neighbourhoods and store them in a list
@st.cache()
...
################################################
# Section 1: Header of the Dashboard
################################################
...
#look up which neighbourhood_group were selected and use it in the function check_borough.
filter_options = check_borough(neighbourhood_group_filter)

# Creates a Multiselectbox for selecting the Neighbourhood. Add an option for "All" which will be defined in the next lines.
neighbourhood_filter = st.sidebar.multiselect("Select Neighbourhood", options=["All"] + sorted(filter_options), default="All")                                      

#after adding the option for all, here we define what the option does. In this case it stores all neighbourhoods in neighbourhood_filter
...
# creating filtered data set according to the inputs
...
################################################
# Section 3: Explore Data Section
################################################
...
#Chart 1
#Creation of a histogram with the price on the x axis and the amount of accomodations on the y axis. ADd a title and update the axis labels
...
#Create a map with pydeck_chart: https://deckgl.readthedocs.io/en/latest/index.html
st.pydeck_chart(
...
st.pydeck_chart(
    ...
    pdk.Deck(
     ...
     initial_view_state=initial_view_state,
     #define the layer(s) on the map. Here the map shows a Iconlayer where each Icon represents one listing in the filtered dataset.
     layers=[         
         pdk.Layer(
             "IconLayer",
            filtered_df,
...
# Functions for the prediction process
#########################################

# Stores the selected values for the prediction in a new dataframe
def create_pred_df():
    d = {'minimum_nights': int(minimum_nights_pred), 'number_of_reviews': int(reviews_pred),
         ...
         'neighbourhood_group': select_borough(neighbourhood_pred),
...
def prepare_pred_df(input_data): 
    # Puts the input data into the df & deletes the other entries 
...
# Creates the button to start the prediction
if st.button("Start Prediction"):
    # Calls the function to get the input data
    input_data = create_pred_df()
    # Calls the function to prepare the df
